...
export default class User extends React.Component {
  ...
  getTransaksiTanggal = (event) => {
    event.preventDefault()
    let url = `http://localhost:4000/cashier/api/transaksi/tanggal/${this.dateAwal(this.state.awal)}/${this.dateAkhir(this.state.akhir)}`
  ...
  pendapatan = () => {
    $("#pendapatan").show()
    $("#btn").hide()
    axios.get("http://localhost:4000/cashier/api/transaksi/detail/", this.headerConfig())
      .then(response => {
    ...
    return (
      <div className='h-screen w-full dark:bg-secondary-bg'>
        ...
        <div>
          <div class="relative shadow-md sm:rounded-lg m-2 pt-32">
            ...
            <div className="m-4 pt-6">
              <form className="flex justify-between items-center" onSubmit={(event) => this.getTransaksiUser(event)}>
              <div className="flex float-right items-center" >
                <div className="relative">
                  <input
                    ...
                    name="awal"
                    type="date"
                    className="bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-text-color dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Select date start"
                  />
                </div>
                <span className="mx-4 dark:text-text-color">Sampai</span>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    ...
                    <input
                      ...
                      name="akhir"
                      type="date"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-text-color dark:focus:ring-blue-500 dark:focus:border-blue-500"
                      placeholder="Select date end"
                    />
                  </div>
                  <button
                    onClick={(event) => this.getTransaksiTanggal(event)}
                    className="p-2.5 ml-2 mr-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-white hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-blue-800"
                  >
                  <svg
                    className="w-5 h-5"
                    fill="none"
        ...
        <div className="flex float-right">
          <button className="py-2.5 mr-3 dark:hover:bg-primary-700 dark:bg-primary-600 dark:text-white text-sm px-4 rounded-lg shadow hover:shadow-md border dark:border-white ease-linear transition-all duration-150" type="button" onClick={() => this.laporan()}>
            Unduh Laporan
          </button>
          <button id="btn" className="py-2.5 mr-3 dark:hover:bg-primary-700 dark:bg-primary-600 dark:text-white text-sm px-4 rounded-lg shadow hover:shadow-md border dark:border-white ease-linear transition-all duration-150" type="button" onClick={() => this.pendapatan()}>
            ...
            <div id="laporan">
              ...
              {/* TOTAL PENDAPATAN */}
              <div className="dark:bg-primary-600 p-2 border-2 mb-2 justify-between items-center hidden" id="pendapatan">
                <p className="font-sans text-xm dark:text-white">Total Pendapatan: {this.convertToRupiah(this.state.pendapatan)}</p>
              </div>
            </div>
        </div>
        </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  ...
                  <tbody>
                    {this.state.detail_transaksi.map(item => (
                      <tr class="border-b dark:border-white font-sans dark:bg-text-color dark:text-white hover:bg-gray-50 dark:hover:bg-gray-600" key={item.id_detail_transaksi}>
                        <td class="px-6 py-4">
                          {item.menu.nama_menu}
                        </td>
                        <td class="px-6 py-4">
